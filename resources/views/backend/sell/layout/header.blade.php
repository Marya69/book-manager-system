                ...
                <ul class="navbar-nav align-items-center gap-1">
                    ...
                    <li class="nav-item dropdown dropdown-large">
                        {{-- <a class="nav-link dropdown-toggle dropdown-toggle-nocaret position-relative" href="#"
                            ...
                            {{-- Give the span an id so JavaScript can target it: --}}
                            <span id="zeroQtyBadge" class="alert-count">0</span>
                            <i class="bx bx-bell"></i>
                        </a>

                        <div class="dropdown-menu dropdown-menu-end"> <br>
                              <p class="text-center text-green-600 mt-2 mr-2">Those books are out of stock.</p>
                            <a href="">
                                <div class="msg-header">
                                   
                                    <p class="msg-header-title">Notifications</p>
                                    <p class="msg-header-badge" id="zeroo"> New</p>
                            ...
                            <div class="header-notifications-list" id="product-notifications">
                                ...
                                <a class="dropdown-item" href="javascript:;">
                                    ...
                                    <div class="d-flex align-items-center">
                                        ...
                                        <div class="flex-grow-1">
                                            <h6 class="msg-name">Daisy Anderson</h6>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        // Fetch zero quantity count
        fetch("{{ route('prodects.zeroQtyCount') }}")
            .then(response => response.json())
            .then(data => {
                const badge = document.getElementById('zeroQtyBadge');
                const badge2 = document.getElementById('zeroo');
                if (badge) badge.textContent = data.count ?? '0';
                if (badge2) badge2.textContent = data.count ?? '0';
            })
            .catch(error => console.error('Error fetching count:', error));

        // Fetch zero-count products
        fetch("/notifications/products")
            .then(response => {
                if (!response.ok) {
            ...
            .then(products => {
                const container = document.getElementById("product-notifications");
                if (!container) {
                    console.error("Missing #product-notifications element in the HTML.");
                    return;
                }
                ...
                if (!Array.isArray(products)) {
                    ...
                    container.innerHTML = '<p class="dropdown-item text-danger">Invalid product format</p>';
                    return;
                }

                if (products.length === 0) {
                    console.warn("No zero-quantity products found.");
                    container.innerHTML = '<p class="dropdown-item text-muted">No low stock products</p>';
                    return;
                }

                products.forEach((product, index) => {
                    if (!product.name || !product.image) {
                        console.warn(`Missing name or image in product at index ${index}:`, product);
                        return;
                    }
                    ...
                    const item = `
                        <a class="dropdown-item" href="javascript:;">
                            <div class="d-flex align-items-center">
                                <div class="user-online">
                                    <img src="/${product.image}" class="msg-avatar"">
                                </div>
                                <div class="flex-grow-1">
                                    <h6 class="msg-name">${product.name}</h6>
                                </div>
                            </div>
                        </a>
                    `;
            ...
            .catch(error => {
                ...
                const container = document.getElementById("product-notifications");
                if (container) {
                    container.innerHTML = '<p class="dropdown-item text-danger">Failed to load products</p>';
                }
            });
    });
</script>
